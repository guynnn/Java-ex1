...
public class Player {
    ...
    /*
     * produces a move for the Random player. Each legal sequence might get picked.
     * @param board the board that we are playing on.
     * @return a valid move for the Random player.
     */
	private Move produceRandomMove(Board board){
		int row = getRandomLegalRow(board);
		int left = getRandomLegalLeft(board, row);
		int right = getRandomLegalRight(board, row, left);
		return new Move(row, left, right);
	}

    /*
     * this function picks a random row that has at least 1 unmarked stick.
     * @param board the board that we are playing on.
     * @return the index of the row
     */
	private int getRandomLegalRow(Board board){
		Random random = new Random();
		while (true) {
            // If this method has been called, it means we have at least one unmarked stick on the board,
            // and the loop will eventually stop.
			int row = random.nextInt(board.getNumberOfRows()) + 1; // 1 <= row <= numbersOfRows
            // Determines weather the row has at least 1 unmarked stick.
			boolean legal = false;
			for (int i = 1; i <= board.getRowLength(row); i++) {
				if (board.isStickUnmarked(row, i)) {
					legal = true; // Row has been found
					break;
				}
			}
			if (legal)
    ...
    /*
     * randomly picks left index in the given row.
     * @param board the board that we are playing on.
     * @param row the row that was randomly picked earlier.
     * @return a valid random index in the given row.
     */
	private int getRandomLegalLeft(Board board, int row){
		Random random = new Random();
		int left = random.nextInt(board.getRowLength(row)) + 1; // 1 <= left <= rowLength
		while (true){
            // If this method has been called, it means we have at least one unmarked stick in the row,
            // and the loop will eventually stop.
    ...
    /*
     * @param board board the board that we are playing on.
     * @param row the row that was randomly picked earlier.
     * @param left the left bound that was randomly picked earlier.
     * @return a valid random index if and only if all the sticks between left and right are unmarked.
     */
	private int getRandomLegalRight(Board board, int row, int left){
		Random random = new Random();
		while (true){
            // If this method has been called, it means we have at least one unmarked stick in the row,
            // and the loop will eventually stop.
			boolean legal = true;
            // left <= right <= rowLength
			int right = random.nextInt(board.getRowLength(row) - left + 1) + left;
			for (int i = left; i <= right; i++){
				if (!board.isStickUnmarked(row, i)){
					legal = false; // not a valid sequence
					break; // try again
				}
			}
			if (legal)
	...
	/*
	 * Produce some intelligent strategy to produce a move. He always make sure there is odd number of
	 * unmarked sticks in the board, thus he removes the first 2 sticks' sequence he finds if there's
	 * currently odd number, and single stick if there's even number. The single sticks picked randomly.
	 */
	private Move produceSmartMove(Board board){
		int numberOfUnmarkedSticks = board.getNumberOfUnmarkedSticks();
		if (numberOfUnmarkedSticks % 2 == 1){
		...
		// even number of unmarked sticks on the board, or there are no 2 sticks' sequence to choose
		// doing the move using the random method.
		int row = getRandomLegalRow(board);
		int left = getRandomLegalLeft(board, row);
		int right = left;
		return new Move(row, left, right);
